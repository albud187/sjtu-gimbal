...
private:
    ...
    std::deque<cv::Mat> recent_templates;
    cv::Mat target_image;
    ...
    void camera_track_cb(const sensor_msgs::msg::Image::SharedPtr msg){
        ...
        if (flt_mode.data=="TRACKING"){
            ...
            // Extract new cropped template
            cv::Mat new_cropped_template = snapshot(cv_track_ptr->image, target_roi_top_left, target_roi_bot_right);

            // Add new cropped template to the queue
            if (recent_templates.size() >= MAX_TEMPLATES) {
                recent_templates.pop_front(); // Remove the oldest template
            }
            recent_templates.push_back(new_cropped_template);

            cv::Mat avg_template = cv::Mat::zeros(new_cropped_template.size(), CV_32FC3); // Float for accurate averaging
            for (const auto& t : recent_templates) {
                cv::Mat temp;
                t.convertTo(temp, CV_32FC3); // Convert to float for summing
                avg_template += temp;
            }
            avg_template /= static_cast<float>(recent_templates.size());

            // Convert back to CV_8UC3 and set as the new template
            avg_template.convertTo(target_image, CV_8UC3);

            // Display the averaged template
            cv::imshow("Averaged Template", target_image);
